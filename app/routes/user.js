...
router.get("/upfilm", function(req, res){
    res.render("upfilm", { data: req.session.user.name, dataAdmin: req.session.userAdmin, email: req.session.email, err: "", pass: "" });
})

router.post("/upfilm", upload.single("poster"), function (req, res) {
    console.log(req.file);
    var params = req.body;
    if(params.ten == "" || params.ngaycongchieu == "" || req.file == null || params.thoiluong == ""){
        res.render("upfilm", { data: req.session.user.name, dataAdmin: req.session.userAdmin, email: req.session.email, err: "true", pass: "" } );
    }
    DB.pool.query('INSERT INTO public."phim"(ten, ngay_cong_chieu, poster, thoi_luong) VALUES ($1, $2, $3, $4)', [params.ten, params.ngaycongchieu, req.file.originalname, params.thoiluong], (err, dulieus) => {
        res.render("upfilm", { data: req.session.user.name, dataAdmin: req.session.userAdmin, email: req.session.email, err: "", pass: "true" } );
    })
  })
